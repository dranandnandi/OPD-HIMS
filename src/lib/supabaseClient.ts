...
export interface Database {
  public: {
    Tables: {
      ...
      patients: {
        refund_requests: {
          Row: DatabaseRefundRequest;
          Insert: Omit<DatabaseRefundRequest, 'id' | 'created_at' | 'updated_at'>;
...
export interface DatabasePaymentRecord {
  ...
  received_by: string;
  clinic_id: string;
  record_type: 'payment' | 'refund' | 'adjustment';
...
export interface DatabaseRefundRequest {
  id: string;
  bill_id: string;
  patient_id: string;
  clinic_id: string;
  source_type: 'bill' | 'pharmacy_dispense';
  total_amount: number;
  refund_method?: 'cash' | 'card' | 'upi' | 'cheque' | 'net_banking' | 'wallet';
  reason?: string;
  ...
  inventory_actions?: Record<string, any>;
  created_at: string;
  updated_at: string;
}
