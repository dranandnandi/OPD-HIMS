...
Team ${clinic.clinicName}`;
  ...
  const handleSendManually = async () => {
    ...
    try {
      // Format phone number for WhatsApp (assuming Indian numbers)
  ...
  const handleSendDirectly = async () => {
    if (!appointment.patient || !clinicSettings) return;
    ...
    try {
      ...
      const response = await fetch(`${import.meta.env.VITE_SUPABASE_URL}/functions/v1/send-blueticks-message`, {
        method: 'POST',
        headers: {
      ...
      setError(err instanceof Error ? err.message : 'Failed to send message via API');
        ...
        <div className="p-6 space-y-6">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            ...
            <button
              ...
              disabled={sendingDirectly || !isBlueticksConfigured || !appointment.patient?.phone}
              ...
              title={!isBlueticksConfigured ? 'Blueticks API key not configured in clinic settings' : ''}
            >
              {sendingDirectly ? (
                <div className="animate-spin rounded-full h-4 w-4 border-b-2 border-white"></div>
              ) : (
          ...
          {!isBlueticksConfigured && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4">
              <div className="flex items-start gap-2">
                ...
                <div>
                  <p className="text-yellow-800 font-medium">Blueticks API Not Configured</p>
                  <p className="text-yellow-700 text-sm mt-1">
                    To use direct sending, please configure your Blueticks API key in the clinic settings.
                  </p>
                </div>
              </div>
            </div>
